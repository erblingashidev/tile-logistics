      ...
      {showForm && (
        <Card className="mb-6 p-4">
          ...
          <form onSubmit={saveEmployee} className="space-y-4">
            ...
            <div>
              <p className="mb-2 text-sm font-medium text-zinc-700">Roles</p>
            ...
            <Card key={e.id} className="p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex gap-1">
                  <Button variant="ghost" className="text-xs" onClick={() => startEdit(e)}>
                    Edit
                  </Button>
                  <Button
                    variant="ghost"
              ...
              {e.roles.includes("driver") && e.assignedVehicle && (
                <p className="mt-2 text-xs text-zinc-500">
                  Truck: {e.assignedVehicle.name} ({e.assignedVehicle.plateNumber})
                </p>
              )}
              {!e.hasLogin && (
                <p className="mt-1 text-xs text-amber-600">No portal login set</p>
              )}
                  ...
                  <ul className="mt-1 space-y-1 text-xs text-zinc-600">
                    {e.assignments.map((a) => (
                      <li key={`${a.orderId}-${a.role}`}>
                        ...
                        {a.vehicleName && (
                          <span className="text-zinc-500"> · {a.vehicleName}</span>
                        )}
                        <span className="text-zinc-400">
                          {" "}
                          · {EMPLOYEE_ROLE_LABELS[a.role as EmployeeRole] ?? a.role}
                        </span>
                      </li>
                    ))}
                  </ul>
