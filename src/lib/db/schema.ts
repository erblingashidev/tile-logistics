...
export const products = sqliteTable("products", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  /** Sellable barcode — often an autogenerated lot code, not a factory EAN. */
  ean: text("ean").unique(),
  productName: text("product_name"),
  ...
  /** Tiles (or pieces) per box/pack. */
  piecesPerPack: integer("pieces_per_pack"),
  m2PerPack: real("m2_per_pack"),
  kgPerPack: real("kg_per_pack"),
  /** Boxes/packs per pallet. */
  packsPerPallet: integer("packs_per_pallet"),
  ...
  /**
   * Groups shade/batch lots of the same commercial type so pack specs can be
   * cloned when a new shipment arrives (color nuance differs by lot).
   */
