...
import { useCompanyProfile } from "@/components/company/CompanyProfileProvider";
import {
  normalizeOrderUnit,
  ORDER_UNIT_LABELS,
...
export function OrderInvoice({ order }: { order: OrderInvoiceData }) {
  ...
  const { organizationName, warehouse } = useCompanyProfile();
  const driverName =
    ...
    <div className="mx-auto max-w-4xl rounded-lg border border-zinc-200 bg-white p-6 shadow-sm print:shadow-none">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-zinc-200 pb-6">
        <div>
          ...
          {warehouse?.address ? (
            <p className="mt-2 text-xs text-zinc-500">{warehouse.address}</p>
          ) : null}
        </div>
